...
export plot_histogram_grad
"""
    plot_histogram_grad(NPB; all,drawnls,fn,fn0,fno,plot_size,lmargin,tmargin,bmargin,rmargin,nbins)
...
# Example
    plot_histogram_grad(77,all=true,fno="histogram_all.pdf")

"""
function plot_histogram_grad(NPB=100::Int64; all=false,drawnls=false,fn="sample.out"::String,fn0="solve.out"::String,fno="histogram.pdf"::String,plot_size=(650,500),lmargin=1.5,tmargin=1.0,bmargin=1.0,rmargin=1.5,nbins=50::Int64,ss=0.5e-4,gd0=0.65,sgd=0.1)
  ...
  for m in 1:MM
    ...
    if drawnls == true
      ...
      if m == 4 || m == 5
        h(x) = 1/sqrt(2*pi*ss^2)*exp(-(x)^2/(2*ss^2))
        plot!(plt,h,lw=2,lc=:orange)
      end
      if m == 6
        gh(x) = 1/sqrt(2*pi*sgd^2)*exp(-(x-gd0)^2/(2*sgd^2))
        plot!(plt,gh,lw=2,lc=:orange)
      end
    end
