...
export forward_all
"""
    forward_all(lat,TR_DEPTH,pos; txtout,fn,fno)                                                                 
...
"""
function forward_all(lat, TR_DEPTH, NPB1, NPB2, NPB3, NPB4; fn1="obsdata_tr.inp"::String,fn2="pxp-ini.inp"::String,fn3="ss_prof.inp"::String,fno="synthetic.txt"::String,fno0="synthetic_NTD.txt",fn4="initial.inp"::String,offsetlntd=8.e-4,siglntd=3.e-4)
  println(stderr," === Generate synthetic dataset  ===")
  NP0 = 11; NC = 18 # Number of fixed parameters
  # --- Input check
  if TR_DEPTH < 0
    error(" TR_DEPTH must be positive")
  end
  if NPB1 < 2
    error(" NPB1 must be more than 2")
  end
  if NPB2 < 2
    error(" NPB2 must be more than 2")
  end                                                                           
  if NPB3 < 1
    error(" NPB3 must be more than 1")
  end
  if NPB4 < 1
    error(" NPB4 must be more than 1")
  end
  # --- Read data
  println(stderr," --- Read files")
  num, nk, tp0, t1, xd1, yd1, zd1, t2, xd2, yd2, zd2, nf, ids = read_obsdata_tr(fn1)
  numk, px, py, pz = read_pxppos(fn2)
  z, v, nz_st, numz = read_prof(fn3,TR_DEPTH)
  NP00, a00, da00, list00 = read_initial_nolimit(fn4)
...
# --- Formatting --- #
  ...
  open(fno,"w") do out
    for n in 1:num
      @printf(out,"%i %2.6f %10.6f %6.6f %6.6f %6.6f %10.6f %6.6f %6.6f %6.6f %d %d\n",nk[n],tp[n],t1[n],xd1[n],yd1[n],zd1[n],t2[n],xd2[n],yd2[n],zd2[n],nf[n],ids[n])
    end
  end
end
