...
* `fn2`: Travel-time residual file name (`fn2="residual_grad.out"` in default)
* `fno`: Output figure name (`fno="gradmap.pdf"` in default)
...
* `ms2`: Plotted marker size for track (`ms2=3` in default)
* `gfs`: Label fontsize (`gfs=12` in default)
* `shape`: Plotted marker shape (`shape=:circle` in default)
